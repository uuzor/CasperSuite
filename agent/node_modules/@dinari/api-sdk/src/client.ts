...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['DINARI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DINARI_LOG'] or 'warn' if it isn't set.
   */
...
export class Dinari {
  ...
  constructor({
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DINARI_LOG'), "process.env['DINARI_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('DINARI_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  protected async authHeaders(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([await this.apiKeyIDAuth(opts), await this.apiSecretKeyAuth(opts)]);
  }

  protected async apiKeyIDAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([{ 'X-API-Key-Id': this.apiKeyID }]);
  }

  protected async apiSecretKeyAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
  ...
  static DinariError = Errors.DinariError;
