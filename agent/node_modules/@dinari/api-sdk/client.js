...
var _Dinari_instances, _a, _Dinari_encoder, _Dinari_baseURLOverridden;
Object.defineProperty(exports, "__esModule", { value: true });
exports.Dinari = void 0;
...
class Dinari {
     ...
     */
    constructor({ baseURL = (0, env_1.readEnv)('DINARI_BASE_URL'), apiKeyID = (0, env_1.readEnv)('DINARI_API_KEY_ID'), apiSecretKey = (0, env_1.readEnv)('DINARI_API_SECRET_KEY'), ...opts } = {}) {
        _Dinari_instances.add(this);
        _Dinari_encoder.set(this, void 0);
        /**
        ...
        if (apiKeyID === undefined) {
            throw new Errors.DinariError("The DINARI_API_KEY_ID environment variable is missing or empty; either provide it, or instantiate the Dinari client with an apiKeyID option, like new Dinari({ apiKeyID: 'My API Key ID' }).");
        }
        if (apiSecretKey === undefined) {
            throw new Errors.DinariError("The DINARI_API_SECRET_KEY environment variable is missing or empty; either provide it, or instantiate the Dinari client with an apiSecretKey option, like new Dinari({ apiSecretKey: 'My API Secret Key' }).");
        }
        const options = {
            apiKeyID,
            apiSecretKey,
        ...
        if (baseURL && opts.environment) {
            throw new Errors.DinariError('Ambiguous URL; The `baseURL` option (or DINARI_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null');
        }
        ...
        this.logLevel =
            (0, log_1.parseLogLevel)(options.logLevel, 'ClientOptions.logLevel', this) ??
                (0, log_1.parseLogLevel)((0, env_1.readEnv)('DINARI_LOG'), "process.env['DINARI_LOG']", this) ??
                defaultLogLevel;
        this.fetchOptions = options.fetchOptions;
        this.maxRetries = options.maxRetries ?? 2;
        this.fetch = options.fetch ?? Shims.getDefaultFetch();
        tslib_1.__classPrivateFieldSet(this, _Dinari_encoder, Opts.FallbackEncoder, "f");
        const customHeadersEnv = (0, env_1.readEnv)('DINARI_CUSTOM_HEADERS');
        if (customHeadersEnv) {
            const parsed = {};
    ...
    async authHeaders(opts) {
        return (0, headers_1.buildHeaders)([await this.apiKeyIDAuth(opts), await this.apiSecretKeyAuth(opts)]);
    }
    async apiKeyIDAuth(opts) {
        return (0, headers_1.buildHeaders)([{ 'X-API-Key-Id': this.apiKeyID }]);
    }
    async apiSecretKeyAuth(opts) {
        return (0, headers_1.buildHeaders)([{ 'X-API-Secret-Key': this.apiSecretKey }]);
    }
    ...
    buildURL(path, query, defaultBaseURL) {
        const baseURL = (!tslib_1.__classPrivateFieldGet(this, _Dinari_instances, "m", _Dinari_baseURLOverridden).call(this) && defaultBaseURL) || this.baseURL;
    ...
    buildBody({ options }) {
        ...
        if (!body) {
            ...
            if (body == null && 'body' in options) {
                return tslib_1.__classPrivateFieldGet(this, _Dinari_encoder, "f").call(this, { body, headers: (0, headers_1.buildHeaders)([rawHeaders]) });
            }
        ...
        ArrayBuffer.isView(body) ||
            ...
            return tslib_1.__classPrivateFieldGet(this, _Dinari_encoder, "f").call(this, { body, headers });
        }
    }
}
exports.Dinari = Dinari;
_a = Dinari, _Dinari_encoder = new WeakMap(), _Dinari_instances = new WeakSet(), _Dinari_baseURLOverridden = function _Dinari_baseURLOverridden() {
    return this.baseURL !== environments[this._options.environment || 'production'];
};
Dinari.Dinari = _a;
Dinari.DEFAULT_TIMEOUT = 60000; // 1 minute
Dinari.DinariError = Errors.DinariError;
Dinari.APIError = Errors.APIError;
Dinari.APIConnectionError = Errors.APIConnectionError;
Dinari.APIConnectionTimeoutError = Errors.APIConnectionTimeoutError;
Dinari.APIUserAbortError = Errors.APIUserAbortError;
Dinari.NotFoundError = Errors.NotFoundError;
Dinari.ConflictError = Errors.ConflictError;
Dinari.RateLimitError = Errors.RateLimitError;
Dinari.BadRequestError = Errors.BadRequestError;
Dinari.AuthenticationError = Errors.AuthenticationError;
Dinari.InternalServerError = Errors.InternalServerError;
Dinari.PermissionDeniedError = Errors.PermissionDeniedError;
Dinari.UnprocessableEntityError = Errors.UnprocessableEntityError;
Dinari.toFile = Uploads.toFile;
Dinari.V2 = v2_1.V2;
